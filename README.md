...
The context API is the solution to constant prop drilling when it comes to working with larger apps with a larger state data. The con of context API is that is can make the reusing of components difficult.

2. In your own words, describe `actions`, `reducers` and the `store` and their role in Redux. What does each piece do? Why is the store known as a 'single source of truth' in a redux application?

Reducers are functions that take two arguments: state and an action object. It consists of a switch statement that consists of action cases where in each case, a logic to manipulate state data, while keeping the original. Actions are objects that consists of a mandatory type property and a payload property if there are any values that are needed to be passed in. The action creator is the function that returns the action object. The store object is known as a 'single source of truth' because it contains the state data for an entire application.

3. What does `redux-thunk` allow us to do? How does it change our `action-creators`?

The redux-thunk allows the redux flow to be asynchronous and the ability to fetch API through action creators. It changes the action creators by returning another function that takes dispatch function as an argument.

4. What is your favorite state management system you've learned and this sprint? Please explain why!

So far, my favorite state management system that I have learned is Redux because it allows the react code to be more DRY (don't repeat yourself). I like the idea that state can be managed in a reducer file, which connected to the Redux store object. Using the useState hook for large applications can be arduous.
